...
function reconcile(records: SourceRecord[], isbn: ISBNInfo): ReconciledBook[] {
  const choices: ReconciledBook[] = [];
  const titleGroups = cluster(records, sameTitle);
  for (const titleRecords of titleGroups) {
    const titleClusterID = makeID("work", [isbn.canonical, normalized(choose(titleRecords, "title"))]);
    for (const editionRecords of cluster(titleRecords, editionCompatible)) {
      ...
      choices.push({
        ...
        isbns: validISBNs([...union(editionRecords, "isbns"), ...isbn.searchForms]),
...
export async function resolveISBN(rawInput: unknown): Promise<Resolution> {
  ...
  const choices = reconcile(deduplicated, isbn);
