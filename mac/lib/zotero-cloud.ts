...
function extra(book: ReconciledBook): string {
  const method = book.isbns.length ? "ISBN reconciliation" : "Title search or physical-book transcription";
  const lines = [`${method}: ${book.confidence}. ${book.reason}`];
...
function item(book: ReconciledBook, collectionKey: string): JsonObject {
  ...
  return {
    ...
    volume: book.volume ?? "",
    numberOfVolumes: book.number_of_volumes ?? "",
    ...
    archive: "",
    archiveLocation: "",
    libraryCatalog: book.source_records.length ? "Book-to-Zotero catalogue reconciliation" : "Physical-book transcription",
    callNumber: "",
    rights: "",
    extra: extra(book),
...
export async function addBook(
  ...
  if (!book.title) throw new ZoteroError("The selected record has no verified title.", 400);
  const collectionKey = requestedCollectionKey.trim();
  if (!validCollectionKey(collectionKey)) throw new ZoteroError("The selected Zotero collection is invalid.", 400);
  const profile = await checkKey(key);
